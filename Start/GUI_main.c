...
static void _DemoTransWindow(void) {
	WM_HWIN hTransWin, hFrameWin , hButton;
	//WM_SetCreateFlags(WM_CF_MEMDEV);
	//WM_EnableMemdev(WM_HBKWIN);
	
	WM_SetDesktopColor(GUI_RED);
//	XDEBUG ("hDesk = %d \n",WM_HBKWIN);
	hTransWin = WM_CreateWindow(20,  160, 72, 52, WM_CF_SHOW | WM_CF_HASTRANS | WM_CF_STAYONTOP, &_cbTransWindow, 0);
//	XDEBUG ("hTransWin = %d \n",hTransWin);
	WM_CreateTimer(hTransWin, 0, 20, 1);    /* Periodic timer for Updates */
	hFrameWin = FRAMEWIN_Create("AAAA", 0, WM_CF_SHOW, 50, 50,600, 500);
//	XDEBUG ("hFrameWin = %d \n",hFrameWin);
	hButton = BUTTON_CreateAsChild (2,20,100,50,hFrameWin,0,WM_CF_SHOW);
//	XDEBUG ("hButton = %d \n",hButton);
...
//	XDEBUG ("GUI START\n");
	GUI_Init();
	WM_SetDesktopColor(GUI_RED);
	//GUI_SetColorIndex(0xff00ff);
	GUI_SetFont(&GUI_Font24_1); 
	GUI_DispStringAt("- - uCGUI disp Function sample - -",100,100); 
