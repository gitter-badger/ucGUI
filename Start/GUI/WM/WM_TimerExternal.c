...
static void _OnTimer(int TimerId) {
  ...
  GUI_HMEM hLink;
  WM_LOCK();
  hLink = _FindTimerByTimerId(TimerId);//找到link节点
  if (hLink) {
    WM_MESSAGE Msg;
    /* Send Message */
    ...
    Msg.hWinSrc = 0;
    WM_SendMessage(pLink->hWin, &Msg);//发送一个消息给回调函数
    /* Since these timers are one shot, we need to delete the link item. */
    _DeleteLinkItem(hLink);// 由于我们创建的是一次性的定时器，那么肯定是响应完了，立马就删掉咯，那要重新启动咋办，再创建咯！！
  }
  WM_UNLOCK();
}
...
void WM_DeleteTimer    (WM_HWIN hWin, int UserId) {
  GUI_HMEM hLink;
  WM_LOCK();
  hLink = _FindTimerByUserId(hWin, UserId);//遍历链表通过ID找link节点
  if (hLink) {
    TIMER_LINK* pLink;
    pLink = GUI_ALLOC_h2p(hLink);
    GUI_X_DELETE_TIMER(pLink->TimerId);	//删除平台定时器
    _DeleteLinkItem(hLink);//删除链表
  }
  WM_UNLOCK();
}
...
int WM_CreateTimer               (WM_HWIN hWin, int UserId, int Period, int Mode) {
  ...
  TIMER_LINK* pLink;
  WM_LOCK();
  TimerId = GUI_X_CREATE_TIMER(Period, _OnTimer);	//创建定时器，依赖平台接口，windows 上windows 的创建接口，linux 上 linux 创建接口
  if (TimerId) {
    hTimerLink = GUI_ALLOC_AllocZero(sizeof(TIMER_LINK));// 申请一块内存，并清0
    if (hTimerLink) {
      pLink = (TIMER_LINK*) GUI_ALLOC_h2p(hTimerLink);
      /* Put new timer at beginning of the linked list */
      pLink->hNext = _hFirst;		//插入定时器链表,这里采用了通用的做法，定时器都是串行操作，不存在race condition 问题
      if (_hFirst) {
        TIMER_LINK* pNext;
      ...
      pLink->UserId  = UserId;
      r = 1;            /* All right, we have successfully created a new timer */ // 好了，我们成功创建定时器了！
    }
  }
  /* Cleanup in case of problem */
  if (r == 0) {
    if (TimerId) {
      GUI_X_DELETE_TIMER(TimerId);// 好吧，保险起见，当创建失败，但是timerID 已经是创建了的时候，那就删除这个平台上的定时器，这个情况就是平台上OK了，但是GUI内存啥的不够了,或者其他出错了
    }
  }
  WM_UNLOCK();
  return r;
