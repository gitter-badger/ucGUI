...
*/
void GUI_Lock(void) {
  if (_EntranceCnt == 0) {// 类似内核内部的 k_reentrant ,记录嵌套次数,如果是第一次lock 
    GUI_X_Lock(); //那么lock 
    _TaskIDLock = GUI_X_GetTaskId();         /* Save task ID */// 记录当前被lock 的ID
  } else {
	  if (_TaskIDLock != GUI_X_GetTaskId()) { // 如果上一个被lock 的ID 和现在的ID 不同
      GUI_X_Lock();		// 那么 lock 
      _TaskIDLock = GUI_X_GetTaskId();         /* Save task ID */ // 重新更新 lock 的ID
    }
  }
  if (++_EntranceCnt == 1) { // 嵌套次数增加，如果是第一次lock 
    int TaskNo = _GetTaskNo();
    if (TaskNo != _CurrentTaskNo) { // 如果发生了 TASK 的切换
      /* Save data of current task */
      if (_CurrentTaskNo>=0) {  /* Make sure _CurrentTaskNo is valid */ //那么保存这个线程的上下文，这个就是有多个线程 wm_exec 的情况
        _Save[_CurrentTaskNo].Context = GUI_Context; // 保存线程UI的上下文
        /* Load data of this task */
        GUI_Context = _Save[TaskNo].Context; 
      }
      _CurrentTaskNo = TaskNo;		//记录当前在运行的 TASK 
    }
  }
  /* Test if _EntranceCnt is in reasonable range ... Not required in release builds */ // 检查嵌套的次数，因为 _Save 数组大小是有限的
